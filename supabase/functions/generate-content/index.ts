...
serve(async (req) => {
  ...
  try {
        ...
        contents: [
          {
            ...
            parts: [
              { text: SYSTEM_PROMPT },
              { text: `Create a beautiful, designer-quality educational card about: ${topic}` }
            ]
          }
        ],
        generationConfig: {
    ...
    // Google Gemini direct API structure: candidates[0].content.parts[0].text
    const content = data.candidates?.[0]?.content?.parts?.[0]?.text || "";

    let cardJson;
    try {
      // Robust cleaning to handle potential markdown artifacts
